...
def create_docu_pages():
    modules_already_added = []
    welcome_page = "* Welcome to WARME ENGINE\n"
    docu_pages_path = Path(output_dir)

    # If Docusaurus pages directory doesn't exist, create it !
    if not docu_pages_path.exists():
        docu_pages_path.mkdir()
    ...
    for warme_module in docs_modules:
        ...
        try:
            ...
            module_path = docu_pages_path / ("Module " + module_name.capitalize())
            ...
            for group in modules_groups:
                if group["title"] == "Classes":
                    for id in group["children"]:
                        ...
                        page_path = module_path / (warme_class["name"] + ".md")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Convert TypeDoc JSON to Docusaurus Files")
    parser.add_argument("--json", "-j", help="Path to the JSON file")
    parser.add_argument("--output", "-o", help="Path to the Folder where Docusaurus will be generated")
