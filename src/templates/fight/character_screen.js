...
class CharacterScreen extends Screen {
  ...
  async onEnter(args = { map }) {
    ...
    for (let i = 0; i < this.numPlayers; i++) {
      ...
      this.uiAvatars[i].src = 'templates/fight/chars/' + 'default-avatar.png';
    ...
    for (let i = 0; i < CHARS.length; i++) {
      this.uiMenu.addWidget(new UIWidget({
        ...
        <div class="UICharacter-container">
          <img class="UICharacter-container-char-icon js-icon" alt="icon" src="${'templates/fight/chars/' + CHARS[i]}/avatar.jpg"/>
        </div>`
      }));
    }
  ...
  async handleMenuItemFocused(data) {
    this.uiAvatars[this.selectionPlayerIndex].src = 'templates/fight/chars/' + CHARS[data.index] + '/avatar.jpg';
  }
  ...
  async handleMenuItemSelected(data) {
    ...
    if (this.selectionPlayerIndex == this.numPlayers) {
      ...
      this.uiAvatars[this.selectionPlayerIndex].src = 'templates/fight/chars/' + CHARS[index] + '/avatar.jpg';
    }
  }
}
