...
class HeroCharacter extends CharacterAbstract {
  ...
  async loadFromData(data) {
    if (data['WeaponId']) {
      ...
      await weapon.loadFromFile('templates/rpg/data/' + data['WeaponId'] + '/data.json');
      this.setEquipment(weapon);
    }
    if (data['HelmetId']) {
      let helmet = new EquipmentItem();
      await helmet.loadFromFile('templates/rpg/data/' + data['HelmetId'] + '/data.json');
      this.setEquipment(helmet);
    }
    if (data['ArmorId']) {
      let armor = new EquipmentItem();
      await armor.loadFromFile('templates/rpg/data/' + data['ArmorId'] + '/data.json');
      this.setEquipment(armor);
    }
    if (data['RelicId']) {
      let relic = new EquipmentItem();
      await relic.loadFromFile('templates/rpg/data/' + data['RelicId'] + '/data.json');
    ...
    this.cast.loadFromFile('templates/rpg/data/cast_' + data['CastId'] + '/data.json');
